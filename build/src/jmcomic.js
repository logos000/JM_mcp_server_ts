...
exports.JmOption = exports.JmDownloader = exports.JmDownloaderImpl = exports.JmOptionImpl = exports.JmImageTool = exports.__version__ = void 0;
        ...
        try {
            ...
            if (isWebP) {
                ...
                try {
                    ...
                    // 转换为JPG
                    (0, child_process_1.execSync)(`ffmpeg -i "${tempWebpPath}" -update 1 -frames:v 1 "${tempJpgPath}"`);
                    // 读取转换后的JPG
...
exports.JmImageTool = JmImageTool;
// 加密工具类
class JmCryptoTool {
    static md5hex(key) {
...
class JmApiClientImpl {
    ...
    async searchComics(params) {
        const { query, page = 1, mainTag = 0, orderBy = 'view', time = 'all' } = params;
        // 参数映射
        const orderMap = {
            'latest': 'mr',
            'view': 'mv',
        ...
        try {
            ...
            const resp = await this.reqApi(`${this.API_SEARCH}?${new URLSearchParams(apiParams).toString()}`);
            const data = resp.resData;
            // 处理直接重定向到专辑的情况
    ...
    async filterByCategory(params) {
        ...
        try {
            ...
            const resp = await this.reqApi(`${this.API_CATEGORIES_FILTER}?${new URLSearchParams(apiParams).toString()}`);
            const data = resp.resData;
            const results = [];
            if (data.content && Array.isArray(data.content)) {
                data.content.forEach((item) => {
